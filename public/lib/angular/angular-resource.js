 ...
 *         'delete': {method:'DELETE'} };
     ...
     * # Buzz client
     ...
     <h1 styles="font-size: 15px;">
     <img src="{{item.actor.thumbnailUrl}}" styles="max-height:30px;max-width:30px;"/>
     <a href="{{item.actor.profileUrl}}">{{item.actor.name}}</a>
     <a href ng-click="expandReplies(item)" styles="float: right;">Expand replies: {{item.links.replies[0].count}}</a>
     </h1>
     {{item.object.content | html}}
     <div ng-repeat="reply in item.replies.data.items" styles="margin-left: 20px;">
     <img src="{{reply.actor.thumbnailUrl}}" styles="max-height:30px;max-width:30px;"/>
